...
if DATABASE_URL:
    if DATABASE_URL.startswith("postgres://"):
        ...
        logger.info("PostgreSQL URL format corrected from 'postgres://' to 'postgresql://'")
...
if use_ssl:
    ...
    logger.info(f"Using SSL for database connection with mode: {ssl_mode}")
...
def create_tables():
    ...
    try:
        if not test_database_connection():
            raise Exception("Could not connect to database, unable to create tables")
